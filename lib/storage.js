...
// Import debug utility (will be undefined in content scripts that don't load it)
const debug = typeof window !== 'undefined' && window.debug ? window.debug : {
  log: () => {},
  error: console.error,
  warn: console.warn
};
...
const Storage = {
  log(...args) {
    debug.info('[RMX-Storage]', ...args);
  },

  error(...args) {
    debug.error('[RMX-Storage ERROR]', ...args);
  },
