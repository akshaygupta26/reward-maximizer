...
const Storage = {
  ...
  async saveOffers(newOffers, source) {
    console.log('[RMX-Storage] ========== saveOffers START ==========');
    console.log('[RMX-Storage] saveOffers called with', newOffers?.length, 'offers from', source);
    console.log('[RMX-Storage] Sample offer:', newOffers?.[0]);
    this.log('saveOffers called with', newOffers?.length, 'offers from', source);

    if (!this.isStorageAvailable()) {
      console.error('[RMX-Storage] ERROR: storage not available');
      this.error('saveOffers: storage not available');
      return { added: 0, updated: 0, total: 0 };
    }

    if (!newOffers || !Array.isArray(newOffers) || newOffers.length === 0) {
      console.error('[RMX-Storage] ERROR: invalid or empty offers array', newOffers);
    ...
    console.log('[RMX-Storage] Merging complete. Total:', mergedOffers.length, 'Added:', added, 'Updated:', updated);
    this.log('saveOffers: merged total:', mergedOffers.length, '| added:', added, '| updated:', updated);

    return new Promise((resolve) => {
      try {
        console.log('[RMX-Storage] Calling chrome.storage.local.set...');
        chrome.storage.local.set({ [StorageKeys.OFFERS]: mergedOffers }, () => {
          if (chrome.runtime.lastError) {
            console.error('[RMX-Storage] CHROME ERROR:', chrome.runtime.lastError.message);
          ...
          console.log('[RMX-Storage] ✅ SUCCESS - Saved', mergedOffers.length, 'offers to storage');
          ...
          chrome.storage.local.get([StorageKeys.OFFERS], (verifyResult) => {
            ...
            console.log('[RMX-Storage] ✅ VERIFIED -', savedCount, 'offers in storage after save');
            this.log('saveOffers: VERIFIED -', savedCount, 'offers in storage after save');
          });

          console.log('[RMX-Storage] ========== saveOffers END ==========');
          resolve({ added, updated, total: mergedOffers.length });
        });
      } catch (err) {
        console.error('[RMX-Storage] EXCEPTION:', err);
