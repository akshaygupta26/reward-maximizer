    ...
    return new Promise((resolve) => {
      try {
        ...
        chrome.storage.local.set({ [StorageKeys.OFFERS]: mergedOffers }, () => {
          if (chrome.runtime.lastError) {
            debug.error('[RMX-Storage] CHROME ERROR:', chrome.runtime.lastError.message);
          ...
          chrome.storage.local.get([StorageKeys.OFFERS], (verifyResult) => {
            ...
            debug.log('[RMX-Storage] ✅ VERIFIED -', savedCount, 'offers in storage after save');
        ...
        debug.error('[RMX-Storage] EXCEPTION:', err);
