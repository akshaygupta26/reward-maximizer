// content/interceptors/bofa-interceptor.js
//
// Bank of America Offers API Observation Layer
//
...
const BofAInterceptor = {
  ...
  inject() {
    ...
    try {
      ...
      scriptEl.remove();
      this._injected = true;
      debug.log('[RMX-Interceptor-BofA] Observer injected into main world');
    } catch (err) {
      debug.warn('[RMX-Interceptor-BofA] Script injection failed (CSP?):', err.message);
    }
  },
  ...
  parseOffers(payload, meta) {
    ...
    try {
      ...
      if (typeof ExtractorConfig !== 'undefined' && ExtractorConfig.logRawResponses) {
        debug.log('[RMX-Interceptor-BofA] Raw sample:', JSON.stringify(rawOffers[0]).substring(0, 500));
      }

      for (const raw of rawOffers) {
        const parsed = this._parseBofAOffer(raw);
        if (parsed) {
          offers.push(BaseInterceptor.normalizeOffer(parsed, this.source));
        }
      }
    } catch (err) {
      debug.warn('[RMX-Interceptor-BofA] Parse error:', err);
    }

    debug.log(`[RMX-Interceptor-BofA] Parsed ${offers.length} offers from API response`);
    return offers;
  },
  ...
  _extractOfferArray(payload) {
    ...
    if (Array.isArray(payload.bankAmeriDeals)) return payload.bankAmeriDeals;
...
if (typeof window !== 'undefined') {
  window.BofAInterceptor = BofAInterceptor;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { BofAInterceptor };
}
