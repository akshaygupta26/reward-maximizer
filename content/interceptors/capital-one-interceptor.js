// content/interceptors/capital-one-interceptor.js
//
// Capital One Offers API Observation Layer
//
...
const CapitalOneInterceptor = {
  ...
  inject() {
    ...
    try {
      ...
      scriptEl.remove();
      this._injected = true;
      debug.log('[RMX-Interceptor-CapitalOne] Observer injected into main world');
    } catch (err) {
      debug.warn('[RMX-Interceptor-CapitalOne] Script injection failed (CSP?):', err.message);
    }
  },
  ...
  parseOffers(payload, meta) {
    ...
    try {
      ...
      if (typeof ExtractorConfig !== 'undefined' && ExtractorConfig.logRawResponses) {
        debug.log('[RMX-Interceptor-CapitalOne] Raw sample:', JSON.stringify(rawOffers[0]).substring(0, 500));
      }

      for (const raw of rawOffers) {
        const parsed = this._parseCapitalOneOffer(raw);
        if (parsed) {
          offers.push(BaseInterceptor.normalizeOffer(parsed, this.source));
        }
      }
    } catch (err) {
      debug.warn('[RMX-Interceptor-CapitalOne] Parse error:', err);
    }

    debug.log(`[RMX-Interceptor-CapitalOne] Parsed ${offers.length} offers from API response`);
    return offers;
  },
...
if (typeof window !== 'undefined') {
  window.CapitalOneInterceptor = CapitalOneInterceptor;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CapitalOneInterceptor };
}
