// content/interceptors/citi-interceptor.js
//
// Citi Offers API Observation Layer
//
...
const CitiInterceptor = {
  ...
  inject(nonce) {
    ...
    try {
      ...
      scriptEl.remove();
      this._injected = true;
      debug.log('[RMX-Interceptor-Citi] Observer injected into main world');
    } catch (err) {
      debug.warn('[RMX-Interceptor-Citi] Script injection failed (CSP?):', err.message);
    }
  },
  ...
  parseOffers(payload, meta) {
    ...
    try {
      ...
      if (typeof ExtractorConfig !== 'undefined' && ExtractorConfig.logRawResponses) {
        debug.log('[RMX-Interceptor-Citi] Raw sample:', JSON.stringify(rawOffers[0]).substring(0, 500));
      }

      for (const raw of rawOffers) {
        const parsed = this._parseCitiOffer(raw);
        if (parsed) {
          offers.push(BaseInterceptor.normalizeOffer(parsed, this.source));
        }
      }
    } catch (err) {
      debug.warn('[RMX-Interceptor-Citi] Parse error:', err);
    }

    debug.log(`[RMX-Interceptor-Citi] Parsed ${offers.length} offers from API response`);
    return offers;
  },
  ...
  async activateAll(offers) {
    debug.log('[RMX-Interceptor-Citi] API activation not yet implemented, deferring to DOM scraper');
    return 0;
  }
};

if (typeof window !== 'undefined') {
  window.CitiInterceptor = CitiInterceptor;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { CitiInterceptor };
}
