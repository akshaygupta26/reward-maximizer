...
const CitiScraper = {
  source: 'citi',
  offersUrl: 'https://online.citi.com/US/ag/merchantoffers',

  needsNavigation() {
    const path = window.location.href.toLowerCase();
    return !path.includes('merchantoffers') && !path.includes('offers');
  },
    ...
    for (const offer of offers) {
      if (offer.button && document.contains(offer.button)) {
        try {
          ...
          console.warn('[Reward Maximizer] Failed to click Citi offer:', err);
        }
      }
    }
  ...
  async scrollToLoad() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const currentCount = document.querySelectorAll('[class*="offer"], [class*="Offer"]').length;
  ...
  findActivateButton(card) {
    ...
    return buttons.find(btn => {
      ...
      const isActivate = /activate|add|enroll|save/.test(text);
      const isAlready = /activated|added|enrolled|saved/.test(text);
      return isActivate && !isAlready;
    });
  },

  extractMerchant(card) {
    const selectors = ['h3', 'h4', 'strong', '[class*="merchant"]', '[class*="name"]', '[class*="title"]'];
  ...
  extractValue(card) {
    ...
    const match = text.match(/(\d+%\s*(?:cash\s*)?back|\$\d+(?:\.\d+)?(?:\s*back)?)/i);
    return match ? match[1] : 'See details';
  },

  extractExpiry(card) {
    const text = card.textContent || '';
    const match = text.match(/(?:expires?|valid|through)\s*:?\s*(\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)/i);
...
if (typeof window !== 'undefined') {
  window.CitiScraper = CitiScraper;
}
